...
class PulseBackendProperties:
    ...
    Extracts backend properties needed by various casq classes and methods.
    ...
    def get_qubit_properties(self, qubit: int):
    ...
    def _get_config(self) -> PulseBackendConfiguration:  # pragma: no cover
        ...
        if hasattr(self.backend, "configuration"):
            ...
            if isinstance(config, PulseBackendConfiguration):
                ...
                raise CasqError(
                    "Backend configuration must support OpenPulse."
                )
        else:
            raise CasqError(
                "Backend must have a configuration method "
                "which returns a 'qiskit.providers.models.PulseBackendConfiguration' instance."
            )
