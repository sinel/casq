...
class CasqSimulator(CasqObject):
    ...
    Wraps and extends :py:class:`qiskit.dynamics.DynamicsBackend`.

    Args:
        backend: Qiskit backend  to use for extracting simulator model.
        (:py:class:`qiskit.providers.BackendV1` or :py:class:`qiskit.providers.BackendV2`)
    """

    @trace()
    def __init__(
        self, backend: Union[BackendV1, BackendV2], qubits: list[int],
    ...
    def run(self, circuit: QuantumCircuit, pulse_map: Optional[list[tuple[str, PulseGate]]] = None) -> DynamicsJob:
        if pulse_map:
            for gate, pulse in pulse_map:
                for instruction in circuit.data:
                    if instruction.operation.name == gate:
                        circuit.add_calibration(gate, instruction.qubits, pulse.schedule)
